...
function Users(){
    ...
    !loading && session && session.admin ? "/api/admin/users" :null,fetcher
    )
    return (
        <>
        <Head>
         <title>FleeFushion | Users</title>
        </Head>
                                    ...
                                    {users.map((user)=>(
                                        <tr key ={`user-${user?._id}`}>
                                         <td className="table_col_img">
                                            <img src={user?.image || '/img/profile_pic.svg'}
                                            className="object-contain w-10 rounded-sm py-2" alt=""/>
                                         </td>
