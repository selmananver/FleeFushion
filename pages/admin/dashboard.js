...
import { ArchiveBoxIcon, PlusIcon, UsersIcon } from "@heroicons/react/24/solid";
import Link from "next/link";

const Dashboard = () => {
  const fetcher = url => fetch(url).then(res => res.json());
  const { data: orders, error } = useSWR("/api/admin/active-orders", fetcher);
  ...
  return (
    <>
      <Head>
        <title>FleeFusion | Dashboard</title>
      </Head>
              ...
              {orders ? (
                orders.length ? (
                  <div className="mt-5 space-y-6">
                    {orders.map(
                      ({ _id, id, amount_total, items, timestamp, order_status }) => (
                        <Order
                          key={`order-${_id}`}
                          id={id}
                          _id={_id}
                  ...
                  <div className="h-full flex items-center justify-center mt-16 sm:w-auto w-3/4 mx-auto sm:max-w-xs">
                    <Image src="/img/empty.svg" width={300} height={300} objectFit="contain" />
                  </div>
                )
              ) : (
                <Skeleton count={12} />
