  ...
  return (
    <div className='w-full py-20 px-6 bg-gray-100 mt-10' id='menu'>
      ...
      <div className='flex items-center w-full max-w-screen-xl sm:mb-20 mb-16 gap-4 mx-auto overflow-x-auto hideScrollBar capitalize text-sm font-medium'>
        <div
          className={`py-2 px-6 bg-red text-center rounded hover:bg-primary-light hover:text-white transition-all cursor-pointer ease-in-out duration-200 shadow ${categoryActive === 'all' ? 'bg-primary-light text-white' : ''}`}
        ...
        {categories?.map((category, i) => (
          <div
            key={`category-${i}`}
            className={`py-2 px-6 bg-red text-center whitespace-nowrap rounded hover:bg-primary-light hover:text-white transition-all cursor-pointer ease-in-out duration-200 shadow ${categoryActive === category?.name ? 'bg-primary-light text-white' : ''}`}
